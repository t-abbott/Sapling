        ...
        with 
            ...
            static member SubWithName (expr : Expr.T) (env : T) (parentNames : Set<Expr.Name>) =
                ...
                | Var name ->
                    ...
                    | None -> Var name
                    | Some ({ contents = Val expr' }) ->
                        expr'
                    | Some ({ contents = Closure (expr', env') }) ->
                        EnvVal.SubWithName expr' env' parentNames
                    | Some ({ contents = envval }) ->
                        Var (envval.ToString ()))
                | LetIn (name, e, body) ->
                    let body' = EnvVal.SubWithName body (env.Without name) (parentNames.Add name) in
                    LetIn(name, e, body')
                | Binop (l, op, r) ->
                    let l', r' = proj2 (fun e -> EnvVal.SubWithName e env parentNames) (l, r) in
                    Binop (l', op, r')
                | IfThen (cond, e1, e2) ->
                    let cond', e1', e2' = proj3 (fun e -> EnvVal.SubWithName e env parentNames) (cond, e1, e2) in
                    IfThen (cond', e1', e2')
                | Fun (name, body) ->
                    let body' = EnvVal.SubWithName body (env.Without name) parentNames in
                    Fun (name, body')
                | Apply (f, x) ->
                    let f', x' = proj2 (fun e -> EnvVal.SubWithName e env parentNames) (f, x) in
                    Apply (f', x')
...
module Eval =
    ...
    let rec eval expr (env : Env.T) =
        match expr with
        | Number _  | Bool _ | Unit -> ref (Env.Val expr)
        | Var v ->
